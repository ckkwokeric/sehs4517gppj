...
<style>
    * {
    ...
    .input-box .message,
    .gender-details .message {
        display: none;
    }

    .input-box .error-message,
    .gender-details .error-message {
        display: block;
        font-size: 14px;
        color: red;
    }
        ...
        <form action="{{ route('registerUser') }}" method="POST">
            ...
            <div class="user-details">
                <div class="input-box">
                    ...
                    <input type="text" class="field @error('fname') error @enderror" placeholder="CHAN" name="fname"
                        value="{{ old('fname') }}">
                    <span class="message error-message">@error('fname') {{ $message }} @enderror</span>
                </div>
                <div class="input-box">
                    <span class="details">Last Name</span>
                    <input type="text" class="field @error('lname') error @enderror" placeholder="Tai Man" name="lname"
                        value="{{ old('lname') }}">
                    <span class="message error-message">@error('lname') {{ $message }} @enderror</span>
                </div>
                <div class="input-box">
                    <span class="details">Email</span>
                    <input type="text" class="field @error('email') error @enderror" placeholder="example@test.com"
                        name="email" value="{{ old('email') }}">
                    <span class="message error-message">@error('email') {{ $message }} @enderror</span>
                </div>
                <div class="input-box">
                    <span class="details">Phone Number</span>
                    <input type="text" class="field @error('phoneNum') error @enderror"
                        placeholder="8-digit phone number" name="phoneNum" value="{{ old('phoneNum') }}">
                    <span class="message error-message">@error('phoneNum') {{ $message }} @enderror</span>
                </div>
                <div class="input-box">
                    <span class="details">Password</span>
                    <input type="password" class="field @error('password') error @enderror" placeholder="Password"
                        name="password" value="{{ old('password') }}">
                    <span class="message error-message">@error('password') {{ $message }} @enderror</span>
                </div>
                <div class="input-box">
                    <span class="details">Date of Birth</span>
                    <input type="date" class="field @error('dob') error @enderror" placeholder="Firstname" name="dob"
                        value="{{ old('dob') }}">
